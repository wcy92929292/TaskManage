layui.config({
	...
	//加载页面数据
	var newsData = '';
	$.get('../../../task/checkingTaskList.do', function(data){
		var newArray = [];
		//单击首页“待审核文章”加载的信息
		...
		if($checkbox.is(":checked")){
            ...
            setTimeout(function(){
            	for(var j=0;j<$checked.length;j++){
            		var dataid = $checked.eq(j).parents("tr").find(".aid").attr("data-aid")
            		$.ajax({
				        type: 'POST',
				        url: '../../../task/updateTaskingStatusOk.do',
				        dataType: 'json',
				        data: {'aid':dataid},//yi个id
				        async: false,
				        success: function (data) {
				        	layer.close(index);
				        	if(data.code=="0"){
		...
		if($checkbox.is(":checked")){
            ...
            setTimeout(function(){
            	for(var j=0;j<$checked.length;j++){
            		...
            		$.ajax({
				        type: 'POST',
				        url: '../../../task/updateTaskingStatusNo.do',
				        dataType: 'json',
				        data: {'aid':dataid},//yi个id
				        async: false,
				        success: function (data) {
...
//					<img style="width: 50px;" src="../../../upload/read.do?name='+currData[i].picture+'" name="'+currData[i].picture+'" onclick=biger(this)>
			    	...
			    	'<td>'+getDateTimeStr(currData[i].startTime)+'</td>'+
			    	'<td class="aid" data-aid="'+currData[i].aid+'">'+getDateTimeStr(currData[i].finishTime)+'</td>'+
//			    	'<td>'
