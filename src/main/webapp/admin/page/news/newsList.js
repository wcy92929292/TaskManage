layui.config({
	...
	//加载页面数据
	var newsData = '';
	$.get("../../../task/getManageTaskList.do",{title:""}, function(data){
		console.log(data)
		var newArray = [];
		//单击首页“待审核文章”加载的信息
...
//			}
			currData = data.result;
			var d1 = new Date();
			var newDate = new Date();
			if(currData.length != 0){
				for(var i=0;i<currData.length;i++){
					newDate.setTime(currData[i].setTime);
					dataHtml += '<tr>'
			    	...
			    	'<td>'+newDate.format('yyyy-MM-dd')+'</td>'
			    	+'<td>'+currData[i].schedule+'</td>'
			    	+'<td>'
...
//			var tid = $(a).
			$(".news_edit").click(function(){
				var tid = $(this).next().attr("data-id");
				var index = layui.layer.open({
					title : "编辑任务",
					type : 2,
					content : "newsEdit.html?tid="+tid,
					success : function(layero, index){
						setTimeout(function(){
							layui.layer.tips('点击此处返回任务列表', '.layui-layer-setwin .layui-layer-close', {
								tips: 3
